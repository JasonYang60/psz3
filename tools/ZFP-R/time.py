...
# DIMENSIONS_LIST=("-3 384 384 256" "-3 384 384 256" "-3 500 500 100" "-1 33554433" "-3 500 500 500" "-1 102953248")
    ...
    eb_base_list = [2e-9]
    eb_list_list = [[1], [65536, 1], [65536, 256,  1], [65536, 4096, 256, 16, 1], [65536, 16384, 4096, 1024, 256, 64, 16, 4, 1], [65536, 32768, 16384, 8192, 4096, 2048, 1024, 512, 256, 64, 32, 16, 8, 4, 2, 1],]
    # eb_list = [256, 16, 1]
    ...
    cnt = 0
    for eb_list in eb_list_list:
        cnt = cnt + 1
        for datasetName, dim, eb_base in zip(dataset_list, dim_list, eb_base_list):

            outPath = './log/zfp_time.log/' + datasetName + '.log.' + str(cnt)
