#ifndef _SZ_SZ_PROGRESSIVE_DELTA_HPP
#define _SZ_SZ_PROGRESSIVE_DELTA_HPP
...
namespace SZ3 {
    ...
    class SZProgressiveDeltaCompressor {
    public:


        SZProgressiveDeltaCompressor(Quantizer quantizer, Encoder encoder, Lossless lossless,
...
//            timer.stop("Interpolation Decompress");
        ...
        uchar *compress(T *data, size_t &compressed_size, bool deleteData) {
            std::cout << "[Log] before quantizing, the size of quant_inds: " << quant_inds.size() << std::endl;

            quant_inds.reserve(num_elements);
            std::cout << "[Log] after reserving, size = " << quant_inds.size() << std::endl;
...
//            preds[0] = 0;
            ...
            std::cout << "[Log] after quant_inds.push_back(...), size = " << quant_inds.size() << std::endl;
            std::cout << "[Log] num_elements = " << num_elements << std::endl;
            ...
            for (uint level = interpolation_level; level > 0 && level <= interpolation_level; level--) {
                ...
                std::cout << "[Log] Level = " << level << ", stride = " << stride << std::endl;
                ...
                for (auto block = inter_begin; block != inter_end; ++block) {
                    ...
                    debug_flag_cnt++;
                    std::cout << "[Log] log info of the " << debug_flag_cnt << "th block:" << std::endl;
                    block.print();

                    block_interpolation(data, block.get_global_index(), end_idx, PB_predict_overwrite,
                                        interpolators[interpolator_id], direction_sequence_id, stride);
                    std::cout << "------[Log]quant element = " << quant_inds.size() << std::endl;
...
//            quant_inds[idx] = quantizer.quantize_and_overwrite(d, pred);
            std::cout << "----[Log] idx = " << idx << std::endl;
            std::cout << "------[Log] pred = " << pred << std::endl;
            std::cout << "------[Log] input d = " << d << std::endl;
            quant_inds.push_back(quantizer.quantize_and_overwrite(d, pred));
            std::cout << "------[Log] output d = " << d << std::endl;
            std::cout << "------[Log] quantize index = " << quant_inds.back() << std::endl;
        }

        inline void recover(T &d, T pred) {
            std::cout << "------[Log] quant_inds[quant_index] = " << quant_inds[quant_index++] << std::endl;
            std::cout << "------[Log] input d = " << d << std::endl;
        ...
        double block_interpolation_1d(T *data, size_t begin, size_t end, size_t stride,
            ...
            if (stride == 1) {
                ...
                std::cout << "--[Log] begin = " << begin << std::endl;
                std::cout << "--[Log] end = " << end << std::endl;
                std::cout << "--[Log] ----- " << std::endl;
            ...
            if (interp_func == "linear" || n < 5) {
                std::cout << "----[Log] linear spline interp... " << std::endl;
                if (pb == PB_predict_overwrite) {
                    std::cout << "----[Log] overwriting... " << std::endl;
                    ...
                    if (n % 2 == 0) {
                        ...
                        std::cout << "----[Log] before quantizing, size = " << quant_inds.size() << std::endl;
                        if (n < 4) {
                            quantize(d - data, *d, *(d - stride));
                        } else {
                            quantize(d - data, *d, interp_linear1(*(d - stride3x), *(d - stride)));
                        }
                        std::cout << "----[Log] after quantizing, size = " << quant_inds.size() << std::endl;
                    }
                } else {
                    for (size_t i = 1; i + 1 < n; i += 2) {
                        T *d = data + begin + i * stride;
                ...
                std::cout << "----[Log] cubic spline interp... " << std::endl;
                if (pb == PB_predict_overwrite) {
