...
namespace SZ3 {
    ...
    class ProgressiveDecomposition : public concepts::DecompositionInterface<T, N> {
     public:
        
        ProgressiveDecomposition(const Config &conf, Quantizer quantizer) : quantizer(quantizer) {
...
//            timer.start();
        ...
        std::vector<int> compress(const Config &conf, T *data, std::vector<int>& segments) {
            std::copy_n(conf.dims.begin(), N, global_dimensions.begin());
            blocksize = 32;
            interpolator_id = conf.interpAlgo;
            direction_sequence_id = conf.interpDirection;
...
//            timer.start();
    ...
    template<class T, uint N, class Quantizer>
    ProgressiveDecomposition<T, N, Quantizer>
    make_decomposition_progressive(const Config &conf, Quantizer quantizer) {
        return ProgressiveDecomposition<T, N, Quantizer>(conf, quantizer);
    }
